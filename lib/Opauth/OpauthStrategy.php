	...
	private function shipToCallback($data, $transport = null){
		...
		switch($transport){
			...
			default:
				...
				//verificar_sesion(); // --> Esta funcion empieza por verificar las sesiones
				$this->imprimir(); //--> Esta funcion imprime el array de opauth
				echo get_bloginfo('url');
				$site_url = parse_url(get_bloginfo('url'));
				print_r($site_url);
				//$this->redirect($this->env['callback_url']); // --> Esta funcion es la inicial del opauth para hacer redirect
		}
	}
	/**
